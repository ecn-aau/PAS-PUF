# -*- coding: utf-8 -*-
"""
Created on Thu Oct 26 11:24:36 2023

@author: Mieszko Ferens

Script to run an experiment for modelling an XOR Arbiter PUF that uses
predictive exhaustive CRP selection during authentication with the server.
"""
...
def convert_int_to_challenge(n, n_bits):
    challenge = np.zeros((1,n_bits), dtype=np.int8) - 1
    index = 0
...
def main():
    ...
    count = 1 # Current CRP to check for viability (skip 0)
    auth_count = 0 # Current number of consecutive successful authentications of the attacker
    
    # Create first query
    for i in range(args.auth_CRPs):
        challenges = np.append(
            challenges, convert_int_to_challenge(count, args.n_bits)).reshape(
                -1, args.n_bits)
        count += 1
    ...
    # Prepare log file
    filepath = Path(
        args.outdir + "out_predictive_exhaustive_CRPs_" + str(args.k) +
        "XOR.csv")
    ...
    for i in range(1, round(args.train_data/args.auth_CRPs)):
        ...
        for crp in range(args.auth_CRPs):
            ...
            for j in range(args.searches):
                ...
                challenge = convert_int_to_challenge(count, args.n_bits)
